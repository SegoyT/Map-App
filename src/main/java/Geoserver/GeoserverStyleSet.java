...
public class GeoserverStyleSet {
	...
	private static String getBodySld(String styleType, final String symbol, final int durchmesser,
		final String fuellfarbe, final String strichfarbe, final int strichstaerke) {
	// TODO SLD Stile anpassbar machen!!
	String bodySld = "";
	...
	} else if (styleType.equals("Polygon")||styleType.equals("MultiPolygon")) {
...
private static String getHeaderSld(final String styleName) {
	...
	final String header = String.format(Locale.US, headerSld, styleNameEsc, styleNameEsc, styleNameEsc);
	return header;
}

private static String getLineSld(final String strichfarbe, final int strichstaerke) {
...
private static String getPointSld(final String symbol, final int durchmesser, final String fuellfarbe) {
	...
	if (noFuellfarbe) {
		...
		sld = String.format(Locale.US, pointSld, symbol, fuellfarbe, durchmesser);
	}
	return sld;

}

private static String getPolygonSld(final String fuellfarbe, final String strichfarbe, final int strichstaerke) {
	final boolean noFuellfarbe = "none".equals(fuellfarbe);
	final boolean noStrichfarbe = "none".equals(strichfarbe);
	String sld = "";
	...
	if (noFuellfarbe) {
		if (noStrichfarbe) {
			...
			sld = String.format(Locale.US, polygonSld, strichfarbe, strichstaerke);
		}
	} else {
		if (noStrichfarbe) {
			sld = String.format(Locale.US, polygonSld, fuellfarbe);
		} else {
			sld = String.format(Locale.US, polygonSld, fuellfarbe, strichfarbe, strichstaerke);
		}
	}
	return sld;
}
...
private static String getRuleHeaderSld(final String beschriftung) {
	//@formatter:off
	final String ruleHeaderSld = "" +
			"        <Rule>\n" +
			"          <Name>%s</Name>\n" +
			"          <Title>%s</Title>\n" +
			"          <Abstract>Adds a geometry</Abstract>\n";
	// @formatter:on
	final String beschriftungEsc = StringEscapeUtils.escapeXml(beschriftung);
	final String ruleHeader = String.format(Locale.US, ruleHeaderSld, beschriftungEsc, beschriftungEsc);
	return ruleHeader;
}
...
public static String doSimpleSLD(String styleName, String styleType, final String beschriftung, final String symbol,
		final int durchmesser, final int strichstaerke, final String strichfarbe, final String fuellfarbe) {
	final String name = styleName;
	final String header = getHeaderSld(name);
	final String ruleHeader = getRuleHeaderSld(beschriftung);
	final String ruleFooter = getRuleFooterSld();
	final String footer = getFooterSld();
	final String body = getBodySld(styleType, symbol, durchmesser, fuellfarbe, strichfarbe, strichstaerke);
